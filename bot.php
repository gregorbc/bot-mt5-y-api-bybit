...
<?php
/**
 * ETH/USDT GRID BOT v13.7 – SCALPING ADAPTATIVO MEJORADO
 * 
 * Cambios respecto a v13.6:
 * - G_MIN_SPACING=0.04% G_MAX_SPACING=0.12% G_SPACING_ATR_MULT=0.20 (scalping 5m)
 * - Añadida persistencia de dirección (cambio solo tras 2 ciclos)
 * - Garantizado notional mínimo de 3 USDT en calcQty()
 * - Timeout MySQL aumentado a 15 segundos
 */
...
// ════════════════════════════════════════════════════════
// 2. CONSTANTES ESTRATÉGICAS (SCALPING ADAPTATIVO MEJORADO)
// ════════════════════════════════════════════════════════
define('G_SYM',           'ETHUSDT');
define('G_CAPITAL',       30.0);
define('G_LEVERAGE',      100);
define('G_CYCLE_SEC',     8);
define('G_AI_INTERVAL',   120);
define('G_TF',            '5');
define('G_CANDLES',       150);
define('G_MIN_LEVELS',    8);
define('G_MAX_LEVELS',    20);
define('G_MIN_SPACING',   0.0004);   // 0.04% — mínimo para scalping 5m
define('G_MAX_SPACING',   0.0012);   // 0.12% — máximo efectivo con ATR ~0.5%
define('G_MARGIN_SAFETY', 0.85);
define('G_MAKER_FEE',     0.0001);
define('G_TAKER_FEE',     0.0006);
define('G_MAX_DAILY_LOSS',12.0);
define('G_HARD_STOP_PCT', 3.0);  // 3% de drawdown no realizado antes de cierre forzoso
define('G_RECOVERY_THR',  1.0);
define('G_COMPOUND_THR',  1.5);
define('G_COMPOUND_MULT', 1.05);
define('G_COMPOUND_CD',   300);
define('G_MIN_NOTIONAL',  3.0);

define('G_FIXED_LEVELS',     16);
define('G_BASE_SPACING',     0.0004);   // 0.04% base para scalping
define('G_SPACING_ATR_MULT', 0.20);     // spacing ≈ ATR% * 0.20 / 100 → ~0.06-0.10% con ATR 0.3-0.5%
define('G_RECOVERY_LOSS_PCT',3.0);
define('G_MIN_BUILD_INTERVAL',60);
define('G_ML_BLEND_WEIGHT',  0.90);
define('G_ML_RELOAD_CYCLES', 120);  // reload check cada 120 ciclos (~16min)
define('G_VL_BLEND_WEIGHT',  0.10);
define('G_VOL_RELOAD_CYCLES', 120);
...
function hGet($url, $timeout = 10) {
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_SSL_VERIFYPEER => true, CURLOPT_USERAGENT => 'EthGridBot/13.7',
    ]);
...
function hPost($url, $payload, $headers = [], $timeout = 25) {
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_TIMEOUT        => $timeout, CURLOPT_USERAGENT => 'EthGridBot/13.7',
...
// ════════════════════════════════════════════════════════
// 6. DATABASE (con timeout aumentado a 15s y reconexión)
// ════════════════════════════════════════════════════════
            ...
            try {
                $pdo = new PDO(
                    ...
                    [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,
                     PDO::ATTR_TIMEOUT => 15,   // aumentado a 15 segundos
...
function dbInit() {
    ...
    lI("[DB] Tablas v13.7 OK");
}

// ════════════════════════════════════════════════════════
// 7. INDICADORES TÉCNICOS (sin cambios)
// ════════════════════════════════════════════════════════
function ema($v, $p) {
...
// ════════════════════════════════════════════════════════
// 8. BYBIT FUTURES API (sin cambios)
// ════════════════════════════════════════════════════════
class BybitFutures {
        ...
        foreach ($sources as $src) {
            try {
                ...
                curl_setopt_array($ch, [
                    ...
                    CURLOPT_USERAGENT      => 'EthGridBot/13.7',
        ...
        try {
            ...
            foreach (isset($r['list']) ? $r['list'] : [] as $acc) {
                ...
                foreach (isset($acc['coin']) ? $acc['coin'] : [] as $c) {
                    if ((isset($c['coin']) ? $c['coin'] : '') !== 'USDT') continue;
...
// ════════════════════════════════════════════════════════
// 9. ML LOCAL (clasificador de dirección) – sin cambios
// ════════════════════════════════════════════════════════
class GridML {
    private $wf;
    ...
    private function load() {
        ...
        foreach ($paths as $f) {
            ...
            $this->accuracy     = (float)(isset($d['acc']) ? $d['acc'] : 0);
        ...
        lW("[ML] Sin archivo de pesos — usando fallback RSI");
    }
...
// ════════════════════════════════════════════════════════
// 10. IA ESTRATÉGICA (respaldo heurístico) – sin cambios
// ════════════════════════════════════════════════════════
class GridAI {
...
// ════════════════════════════════════════════════════════
// 11. FUNCIÓN DE ANÁLISIS CON NVIDIA VL (sin cambios)
// ════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════
// 12. GRID MANAGER (con mejoras de espaciado, persistencia de dirección y notional mínimo)
// ════════════════════════════════════════════════════════
class GridManager {
    private $api;
    private $ai;
    ...
    // Variables para persistencia de dirección
    ...
    public function __construct($api, $ai, $ml) {
        ...
        $this->ai = $ai;
        $this->ml = $ml;
        $this->volFile = __DIR__ . '/volatility_weights.json';
    ...
    private function loadVolatilityModel() {
        // Preferir ridge model (tiene clip bounds calibrados de datos de entrenamiento)
        $ridgeFile = __DIR__ . '/volatility_weights_ridge.json';
        $candidates = [$ridgeFile, $this->volFile];
        $chosen = null;
        foreach ($candidates as $f) { if (file_exists($f)) { $chosen = $f; break; } }
        if (!$chosen) {
        ...
        // Cargar clip bounds si existen (ridge model los incluye)
    ...
    private function reloadVolatilityIfUpdated() {
        ...
        if ($mtime > $this->volMtime) {
            lI("[Vol] Detectada actualización de volatility_weights.json, recargando...");
            $this->loadVolatilityModel();
        }
    }

    // ─── PREDICCIÓN DE ATR CON AJUSTE ADAPTATIVO (sin rechazo) ───
    private function predictFutureATR($candles) {
        ...
        // Si la predicción es negativa, el modelo está fuera de distribución — ignorar
        if ($pred_original < 0) {
            lW(sprintf("[Vol] Pred negativa (%.4f) — modelo fuera de rango, usando ATR actual %.2f%%", $pred_original, $atr_actual));
            return null;
        }
        
        // Usar clip bounds del ridge model si están disponibles (más calibrado)
        $clipLo = isset($this->volClipLower) ? $this->volClipLower : 0.05;
        $clipHi = isset($this->volClipUpper) ? $this->volClipUpper : 1.5;
        $pred = max($clipLo, min($clipHi, $pred));
        
        // Ajuste adaptativo suave
        if ($atr_actual > 0.01) {
            ...
            if ($ratio < 0.5) {
                ...
                $pred = 0.7 * $atr_actual + 0.3 * $pred;
                lW(sprintf("[Vol] Pred alta (ratio %.2f), blend suave: %.4f%%", $ratio, $pred));
            }
        }
    ...
    public function run() {
        lI("╔══════════════════════════════════════════╗");
        lI("║  ETH/USDT Grid Bot v13.7 – SCALPING     ║");
        ...
        while ($this->running) {
            ...
            try {
                ...
                if (time() - $this->lastAI >= G_AI_INTERVAL) $this->aiEvaluate($price);
    ...
    private function loadConfig() {
        ...
        if (!$row) {
            dbx(function($d) { return $d->prepare("INSERT INTO grid_configs (symbol,direction,confidence,capital_usd,leverage,levels,spacing_pct,long_levels,short_levels,qty_per_level,pp,qp,mode) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE status='ACTIVE'")
                ->execute([G_SYM, 'SIDEWAYS', 50, G_CAPITAL, G_LEVERAGE, G_FIXED_LEVELS,
                           G_BASE_SPACING, (int)(G_FIXED_LEVELS/2), (int)(G_FIXED_LEVELS/2), 0, 2, 2, 'NORMAL']); });
        ...
        lI(sprintf("[CFG] niv=%d spc=%.4f%% long=%d short=%d capital=%.0f",
            ...
            isset($row['long_levels']) ? $row['long_levels'] : (G_FIXED_LEVELS/2), isset($row['short_levels']) ? $row['short_levels'] : (G_FIXED_LEVELS/2), G_CAPITAL));
    }

    // ─── LIMPIEZA CON REINTENTOS ───
    private function cleanupSession() {
        ...
        for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
            ...
            foreach ($positions as $p) {
                $sz = abs($p['positionAmt'] ?? 0);
                if ($sz > 0.001) $remaining++;
            }
            if ($remaining == 0) {
                lI("[INIT] Todas las posiciones cerradas (intento $attempt)");
                break;
            }
            lW("[INIT] Quedan $remaining posiciones, reintentando cierre en 2s...");
            sleep(2);
        }
        dbx(function($d) {
        ...
        lI("[INIT] Sesión limpiada. Reconstruyendo grilla...");
    }

    // ─── CIERRE DE POSICIONES CON REINTENTOS INDIVIDUALES ───
    ...
    private function applyAIResultFallback($price) {
        ...
        dbx(function($d) use ($direction, $confidence, $reason, $levels, $spacing, $qty) {
            return $d->prepare("UPDATE grid_configs SET direction=?,confidence=?,ai_reason=?,last_ai_check=NOW(),levels=?,spacing_pct=?,long_levels=?,short_levels=?,qty_per_level=? WHERE symbol=?")
                ->execute([$direction, $confidence, $reason, $levels, $spacing, $levels/2, $levels/2, $qty, G_SYM]);
        });
    ...
    // ─── EVALUACIÓN IA CON ESPACIADO BASADO EN ATR BLEND SIEMPRE Y PERSISTENCIA DE DIRECCIÓN ───
    private function aiEvaluate($price) {
        ...
        if ($direction !== $prevDir) {
            if ($direction === $this->lastDirection) {
                ...
                $direction = $prevDir; // aún no cambiamos
                $confidence = (int)round(($confidence + (isset($this->cfg['confidence']) ? $this->cfg['confidence'] : 50)) / 2);
            }
        } else {
            $this->directionChangeCount = 0;
        ...
        // Fórmula: spacing = BASE + ATR_efectivo * MULT / 100
        // Con ATR=0.30%: 0.0004 + 0.30*0.20/100 = 0.0004+0.0006 = 0.0010 (0.10%)
        // Con ATR=0.50%: 0.0004 + 0.50*0.20/100 = 0.0004+0.0010 = 0.0014 → clamp 0.0012
        $spacing_raw = G_BASE_SPACING + ($atr_efectivo * G_SPACING_ATR_MULT / 100);
        $spacing = min(G_MAX_SPACING, max(G_MIN_SPACING, $spacing_raw));
        // SIDEWAYS: reducción 10% para más fills
        ...
        // Mostrar atr_pred de forma legible (null → "null")
    ...
    // calcQty con log reducido y garantía de notional mínimo
    ...
    private function calcQty($price, $levels, $f, $knownBalance = null) {
        ...
        $maxNotional = ($effectiveCap * 0.04) * G_LEVERAGE;
        $maxQty = $maxNotional / $price;
        if ($qty > $maxQty) $qty = $maxQty;
        
        $qty = max($f['step'], round($qty / $f['step']) * $f['step']);
        $notional = $qty * $price;
        // Garantizar notional mínimo (3 USDT)
        ...
        // Solo loguear si cambia respecto al último valor logueado
    ...
    private function buildGrid($price) {
        ...
        $longLev  = (int)(isset($cfg['long_levels']) ? $cfg['long_levels'] : (G_FIXED_LEVELS/2));
        $shortLev = (int)(isset($cfg['short_levels']) ? $cfg['short_levels'] : (G_FIXED_LEVELS/2));
    ...
    private function onFill($order, $info, $price) {
        ...
        if ($role === 'ENTRY') {
            ...
            if ($positionFound) {
                ...
                lW(sprintf("[FILL] ENTRY %s $%.2f sin posición → reciclando", $side, $fillPx));
    ...
    private function confirmPositionExists($entrySide, $minQty) {
        $sleepMs = [1000000, 1500000, 2000000];
        for ($i = 0; $i < 3; $i++) {
            $positions  = $this->api->positions(G_SYM);
            $targetSide = ($entrySide === 'BUY') ? 'Buy' : 'Sell';
            foreach ($positions as $pos) {
                if ((isset($pos['side']) ? $pos['side'] : '') === $targetSide) {
                    $sz = abs(isset($pos['positionAmt']) ? $pos['positionAmt'] : (isset($pos['size']) ? $pos['size'] : 0));
                    if ($sz >= $minQty * 0.99) return true;
                }
            }
            if ($i < 2) usleep($sleepMs[$i]);
        }
        return false;
    }
    ...
    // === Funciones de riesgo, recovery, optimización, breakout (sin cambios relevantes) ===
    ...
    private function profitOptimize($price) {
        ...
        if ($pct >= G_COMPOUND_THR && !(isset($this->cfg['recovery_active']) && $this->cfg['recovery_active']) && $cooldownOk) {
            ...
            $baseQty = $this->calcQty($price, $levels, $f);
            $newQty  = min($baseQty * G_COMPOUND_MULT, (G_CAPITAL * G_LEVERAGE * 0.8 / $levels) / $price);
    ...
    private function breakoutCheck($price) {
        ...
        $r = dbx(function($d) {
            return $d->query("SELECT MIN(price) mn,MAX(price) mx FROM grid_orders WHERE symbol='" . G_SYM . "' AND status='OPEN' AND grid_role='ENTRY'")->fetch();
        });
        if (!$r || !$r['mn']) return;
        $range = (float)$r['mx'] - (float)$r['mn']; $margin = $range * 0.18;
    ...
    private function writeStatus($price) {
        ...
        file_put_contents($STATUS, json_encode([
            'ts' => date('Y-m-d H:i:s'), 'mode' => $mode, 'ai_engine' => 'Grid v13.7',
            ...
            'pairs' => [G_SYM => [
                ...
                'long_levels'    => (int)(isset($cfg['long_levels']) ? $cfg['long_levels'] : (G_FIXED_LEVELS/2)),
                'short_levels'   => (int)(isset($cfg['short_levels']) ? $cfg['short_levels'] : (G_FIXED_LEVELS/2)),
