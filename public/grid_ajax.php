<?php
/**
 * grid_ajax.php v14.8 – ETH/USDT Grid Bot Backend
 * Endpoints: _status, _logs, _ticker, _market, _control, _pnl_float,
 *            _ai_decisions, _scalp, _ml_info, _fills_history
 * 
 * CORREGIDO: logs siempre muestran algo (diagnóstico si no hay reales)
 * Incluye campos: atr_predicted, vl_used, vl_direction, vl_confidence
 * 
 * MODIFICADO v14.9: dbInit() para crear tablas automáticamente
 */
error_reporting(0);
...
// Para datos de mercado SIEMPRE mainnet (testnet no tiene klines históricas ni precios reales)
...
// Nueva función: crear tablas si no existen
function dbInit(PDO $db): void {
    try {
        ...
        // Asegurar columna ml_accuracy en grid_configs (por si se añade después)
        try { $db->exec("ALTER TABLE grid_configs ADD COLUMN ml_accuracy DECIMAL(6,4) DEFAULT 0"); } catch (Exception $e) {}
    } catch (Exception $e) {
        // No hacer nada, la tabla ya existe o error transitorio
    }
}

function botRunning(string $pidFile, string $logFile): bool {
    // Buscar PID en múltiples ubicaciones (bot escribe en public_html/, config puede apuntar a private/)
    ...
    // Fallback: log actualizado en los últimos 90s
...
if (isset($_GET['_ticker'])) {
    $t = getBybitTicker($pubBase, 'ETHUSDT');  // siempre mainnet
...
// ═══════════════════════════════════════════════════════
// 2. BOT STATUS (con token) – incluye campos ATR y VL
// ═══════════════════════════════════════════════════════
...
if (isset($_GET['_market'])) {
    // Market data: SIEMPRE mainnet (testnet no tiene datos históricos)
    $t  = getBybitTicker($pubBase, 'ETHUSDT');
    $fn = getBybitFunding($pubBase, 'ETHUSDT');
    $oi = getBybitOI($pubBase, 'ETHUSDT');

    // Intentar Binance FAPI primero (más confiable), luego Bybit mainnet
    $klines = []; $closes = []; $volumes = []; $highs = []; $lows = [];
    $klSources = [
    ...
    foreach ($klSources as $src) {
        ...
        if ($src['type'] === 'binance') {
            ...
            foreach ($kd as $k) {
                ...
                $highs[]  = (float)$k[2]; $lows[]    = (float)$k[3];
            }
        } else {
            $kd = json_decode($resp, true);
    ...
    // Klines cargados arriba via Binance/Bybit
    ...
    // MACD Hist
    $ema12=[];$ema26=[];$macdLine=[];$macdHist=0.0;
    $k2=2/13; $k26=2/27; $k9=2/10;
    if (count($closes)>=26) {
        $e12=array_sum(array_slice($closes,0,12))/12;
        $e26=array_sum(array_slice($closes,0,26))/26;
        for ($i=12;$i<count($closes);$i++){$e12=$closes[$i]*$k2+$e12*(1-$k2);$ema12[]=$e12;}
...
// ═══════════════════════════════════════════════════════
// 4. LOGS (con token) – CORREGIDO: siempre muestra algo
// ═══════════════════════════════════════════════════════
if (isset($_GET['_logs'])) {
    $lines = [];
    ...
    // Si no hay líneas reales, inyectamos logs de diagnóstico
    if (empty($lines)) {
        $now = date('Y-m-d H:i:s');
        $lines[] = "$now [INFO] === Dashboard v14.2 ===";
        $lines[] = "$now [INFO] Esperando logs del bot...";
        if (!$logExists) {
            $lines[] = "$now [WARN] Archivo de log no encontrado en: $logFile";
            $lines[] = "$now [WARN] Verifica que bot.php se haya ejecutado y tenga permisos de escritura.";
        } else {
        ...
        $lines[] = "$now [INFO] También puedes forzar una entrada de prueba con: echo 'test' >> $logFile";
    }
...
echo json_encode(['error' => 'no action', 'version' => '14.9']);
?>